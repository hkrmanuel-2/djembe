/**
 * Create test accounts for Jesus and Mary School
 *
 * Usage:
 *   1. Add SUPABASE_SERVICE_ROLE_KEY to your .env.local file
 *   2. Run: node scripts/create-jesus-mary-accounts.js
 *
 * Creates:
 *   - 1 teacher account
 *   - 12 student accounts
...
const SCHOOL_NAME = "Jesus and Mary School";
const EMAIL_DOMAIN = 'jesusmary.test';
...
async function main() {
  console.log('=== Creating Test Accounts for Jesus and Mary School ===\n');
  ...
  if (!cls) {
    const { data, error } = await supabase
      ...
      .insert({
        ...
        grade_level: 'Primary',
        description: 'Music class for Jesus and Mary School',
      })
      .select('class_id')
      .single();
